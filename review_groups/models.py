...
class ReviewGroups(models.Model):
    ...
    Each group has a unique identifier (group_id) and is associated with a name (group_name).
    A group can have a description (group_description) providing additional information.
    It includes a ManyToMany relationship with movies (group_movies) and users (group_members).
    The creator of the group is tracked through the ForeignKey relationship (group_creator).

    Methods:
        - __str__: Returns the group_name for a human-readable representation.
    """
